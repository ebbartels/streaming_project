...
CREATE OR REPLACE TABLE streaming_pipeline.trains.active_trains_by_line AS
    SELECT line, count(DISTINCT train_id) as total_rows
    FROM streaming_pipeline.trains.train_data_silver
    WHERE service_type = 'Normal'
    AND TIMESTAMPDIFF('minute', created_timestamp, current_timestamp()) <= 10 
    GROUP BY line
...
-- test query for active trains
SELECT * 
FROM streaming_pipeline.trains.active_trains
;

-- test query for active trains by line
SELECT * 
FROM streaming_pipeline.trains.active_trains_by_line
;